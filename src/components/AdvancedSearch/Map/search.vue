<template>
  <div class="bg" style="overflow: auto;height: 100%;margin: 0px 30px;width: 1200px">
    <div class="search-wrapper" >
      <el-input id="search_input2"  v-model="search_val" placeholder="请根据企业名称，经营范围，业务商品等关键词内容搜索" class="input-with-select">
    ...
    <div class="cond-wrapper" style="background-color: white;margin-top: 10px;border-radius: 5px;border: 1px solid #DFDFDF;">
      ...
      <el-button id="gengduo" type="primary" style="background-color: #006EFF;margin: 0px 25px" @click="contrl_cond_down_show">更多筛选项目</el-button>
...
import {
  advancedSearch, advancedSearchOption,
...
export default {
  ...
  mounted() {
    ...
    advancedSearchOption(this.query).then(res => {
      const list = []
    ...
    var searchDiv = document.getElementById('search_input2').nextElementSibling
    ...
    this.$el.getElementsByClassName('el-drawer__body').item(0).style.overflow = 'auto'
  },
  methods: {
    updatezupu() {
      this.drawLine()
    ...
    submitForm(page) {
      ...
      advancedSearch(this.searchQuery).then(res => {
        var searchText = document.getElementById('search_input2').value
