<template>
  <div class="box">
    ...
    <div v-if="detail.length !== 0" class="box2">
      ...
      <el-row class="cont">
        <el-col :span="12"><div class="grid-content bg-purple" />
          <span>执行号：</span><span>{{ detail[0].yjCode }}</span>
        </el-col>
        <el-col :span="12"><div class="grid-content bg-purple" />
          <span>执行法院：</span><span>{{ detail[0].yjdw }}</span>
        </el-col>
      </el-row>
      <el-row class="cont">
...
<script>
import { getShixinDetail } from '@/api/JudicialDecisions'
export default {
  data() {
    return {
      query: {
  ...
  created() {
    ...
    getShixinDetail(this.query).then(res => {
    //   console.log(res)
